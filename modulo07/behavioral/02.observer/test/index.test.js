...
describe('Test Suite for Observable Pattern', () => {
    ...
    test('#PaymentSubject should notify observers after a credit card transaction', () => {
        const subject = new PaymentSubject()
        ...
        const data = { userName: 'Jao', id: Date.now() }
    ...
    test('#All should notify subscribers after a credit card payment', () => {
        ...
        const payment = new Payment(subject)
        const data = { userName: 'JaoFlutterBoy', id: Date.now() }
        payment.creditCard(data)
