...
/* const results = ['1', '2'].map(async (item) => {
    console.log('starting process!! - MAP')
    await timeoutAsync(100)
    console.log(item)
    console.log(await Promise.resolve('timeout order! - MAP'))
    await timeoutAsync(100)
    console.log(item)
...
setTimeout(async () => {
    ...
    await Promise.reject('Yes, that\'s right')
})
...
try {
    console.log('hello')
    console.log('world')
    throwError('Error boy')
} catch(e) {
    console.log('catch error guys', e.message)
} finally {
